...
class _NotesInboxScreenState extends State<NotesInboxScreen> {
  ...
  void initState() {
    super.initState();
    _notesBloc = sl<NotesBloc>()..add(LoadNotesForChildren(widget.childIds));
  }

  /// عند الخروج من الشاشة — علّم كل الملاحظات المعروضة كمقروءة
  void _markAllReadOnExit() {
    for (final id in widget.childIds) {
      _notesBloc.add(MarkAllNotesRead(id));
    }
  }
                  ...
                  return RefreshIndicator(
                    onRefresh: () async {
                      context.read<NotesBloc>().add(
                        LoadNotesForChildren(widget.childIds),
                      );
                    },
