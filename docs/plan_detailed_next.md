...
### 1.2 Realtime — كود Flutter (أولوية عالية)
الـ migration (007) فعّلت النشر فقط؛ التطبيق لا يتحدّث تلقائياً بدون كود.

| المهمة | التفصيل | الملفات/الموقع المقترح |
|--------|---------|--------------------------|
| خدمة Realtime | إنشاء `RealtimeService` (أو مسمّى موحّد) تشترك في قنوات Supabase `postgres_changes` للجداول: `attendance`, `mosques`, `correction_requests`, `notes`. | مثلاً `lib/app/core/services/realtime_service.dart` |
| اشتراك الحضور | عند فتح الشاشة الرئيسية لولي الأمر: الاشتراك في تغييرات `attendance` (مرشّح بـ `child_id` لأطفال المستخدم أو بـ مسجد أطفاله). عند استلام INSERT/UPDATE → تحديث state "حضور اليوم" أو إعادة جلب البيانات. | الشاشة الرئيسية لولي الأمر + Bloc/Cubit إن وُجد |
| اشتراك موافقة المسجد | عند فتح بوابة المسجد أو لوحة الإمام: الاشتراك في تغييرات `mosques` (مرشّح بمسجد المستخدم). عند UPDATE لـ `status` → تحديث state أو إعادة جلب المساجد. | بوابة المسجد، لوحة الإمام |
| إلغاء الاشتراك | عند `dispose` لكل شاشة معنية إلغاء الاشتراك في القناة لتقليل الاستهلاك والبattery. | نفس الملفات أعلاه |
| سحب يدوي (بديل) | إضافة Pull-to-refresh في قوائم "حضور اليوم" وطلبات المساجد كبديل حتى اكتمال Realtime أو عند فشل الاتصال. | الشاشات المعنية |
