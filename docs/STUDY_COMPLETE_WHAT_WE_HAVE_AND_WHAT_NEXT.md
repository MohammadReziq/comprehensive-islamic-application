...
| **أولوية قادمة** | (1) تحقق وقت الحضور + شاشة طلب تصحيح (2) الإمام يحدد موقع المسجد (3) عرض الابن للباركود/ربط جهاز الابن (4) المسابقة النشطة لولي الأمر. **المرجع الكامل:** PLAN_FINAL_COMPLETE.md |
...
| **واجهة موقع المسجد للإمام** | الموديل يحمل lat/lng؛ لا شاشة للإمام لتعيين الموقع؛ بدونها أوقات الصلاة تبقى مكة. | عالية (ضمن الخطة القريبة) |
| **عرض الابن للباركود / ربط جهاز الابن** | الابن لا يملك طريقة لعرض باركوده من جواله؛ تحتاج كود ربط وشاشة "عرض الابن". | عالية (ضمن الخطة القريبة) |
| **أوقات الصلاة حسب المسجد** | استخدام lat/lng المسجد (من 022) في PrayerTimesService عند التحضير؛ مرتبط بواجهة موقع المسجد أعلاه. | متوسطة |
...
3. **الإمام يحدد موقع المسجد + ربط أوقات الصلاة بالمسجد**  
   - واجهة للإمام: إعدادات المسجد (أو عند الإنشاء) تحتوي **الموقع (lat/lng)** — خريطة أو "استخدام موقعي الحالي" أو إدخال يدوي. حفظ في mosques.  
   - استخدام lat/lng المسجد في PrayerTimesService عند فتح التحضير (`getTimesForMosque(mosqueId)`؛ fallback مكة).  
   - بدونها أوقات الصلاة تبقى مكة افتراضياً.

4. **عرض الابن للباركود / ربط جهاز الابن**  
   - ولي الأمر من بطاقة الطفل → "ربط جهاز الابن" → كود ربط (مؤقت). الابن يفتح التطبيق → "لدي كود ربط" ويدخل الكود → التطبيق يربط الجهاز بالطفل (تخزين محلي أو جدول).  
   - شاشة "عرض الابن" للجهاز المربوط: الاسم، الباركود، (اختياري) حضور اليوم ونقاطي — دون حساب ولي أمر.  
   - التفاصيل الكاملة (ملفات، تدفق، UX) في **PLAN_FINAL_COMPLETE.md**.
...
6. **Realtime لطلبات التصحيح والملاحظات**  
...
8. **رسائل خطأ واضحة (AppFailure → واجهة)**  
...
# جزء إضافي: مؤجّل فقط (مرجع سريع)

البنود **موقع المسجد (الإمام)** و**عرض الابن للباركود / ربط جهاز الابن** صارت **جزءاً من الخطة الفعلية** (المرحلة القريبة والتالية) — التفاصيل الكاملة في **PLAN_FINAL_COMPLETE.md**.

**ما يبقى مؤجّلاً (لاحقاً):**
...
| **المشرف لا يسجّل صلاة إلا ضمن وقتها** | يُطبّق تلقائياً عند تنفيذ **AttendanceValidationService** (مرحلة حرجة). |
| **الإعلانات، مسجد أساسي/ثانوي (واجهة)، Offline، FCM، الجوائز/الشارات** | كما في جدول "مؤجّل أو لاحق" أعلاه. |
...
| **product_ux_notes.md** | ربط جهاز الابن، من لا يسجّل، خيار "عرض الابن للباركود". |
| **MASTER_PLAN_CRITICAL_REVIEW.txt** | نقد الأدوار، موجات التنفيذ، migrations وملفات مطلوبة. |
| **PLAN_FINAL_COMPLETE.md** | **الخطة النهائية المتكاملة** — مرجع واحد لما سنفعله بالخطة القادمة (حرج → قريب → تالي → مؤجّل)، تفاصيل تنفيذية، ورحلة مستخدم. |

---

*هذه الدراسة تجمع ما لدينا وما هو قادم في مكان واحد؛ الخطة المفصّلة والنهائية في **PLAN_FINAL_COMPLETE.md**. يُحدَّث عند إنجاز بنود أو حسم قرارات.*
